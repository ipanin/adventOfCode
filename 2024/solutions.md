...
## day 10
Дана матрица высот. Маршруты начинаются на высоте 0, на каждом шаге на 4 соседних клетки увеличиваются на 1 и заканчиваются на 9.
Часть 1. Посчитать количество вершин (9), в которые можно прийти из каждого начала (0).
Часть 2. Посчтитать количество различных маршрутов, по которым можно прийти из 0 в 9.
...
## day 16
Поиск дешевого пути в лабиринте. Шаг стоит 1р, поворот на 90 градусов стоит 1000р.
...
**Решение**: Берем каждое полотенце, подходящее к началу кода и рекурсивно считаем сколько вариантов для остатка кода.
Для ускорения используется мемоизация.
...
## day 21
Даны ПИН-коды. Набрать их с помощью цепочки из 3-х (26) роботов-посредников.
**Решение:** найти все варианты ввода цифрового кода. Для каждого варианта посчитать длину кода, полученного после 2х (25) преобразований. Длина кода считается рекурсивно для каждой подпоследовательности, заканчивающейся на А. Для ускорения нужно использовать мемоизацию. Для преобразования последовательности направлений можно использовать 1 вариант трансформации.

## day 22
Дан список чисел. Они являются seed для псевдослучайных последовательностей.

**Часть 1.** Найти сумму 2000х элементов каждой из последовательностей.
**Решение:** в лоб.

**Часть 2.** В каждой последовательности 2000 псевдослучайных чисел (см. часть 1). Последняя цифра в каждом числе это цена в бананах. 4 последовательных изменения цены - это шаблон. Цена покупки - это следующее значение после шаблона. Подобрать шаблон так, чтобы сумма всех цен покупки была максимальной.
**Решение:** На входе 2000*1774 ~ 3_500_000 цен. Вариантов шаблонов 19**4 ~ 160000. Всего перебрать 462_378_908_000. Это слишком много. Поэтому сократим число проверяемых шаблонов. Будем считать, что подходящий шаблон приводит хотя бы раз к цене 9. Всего шаблонов, приводящих к 9 оказалось 10 000. Возьмем из них 1000 шаблонов, которые привели к 9 чаще всего. Перебрать 1000 шаблонов можно за 1 минуту.
